...
<h1 class="panel panel-default title-page">
	<span class="panel-body display-block">
		...
		<s:elseif test="getStrutsAction() == 2">
			<s:text name="jpjenkins.params.label.edit" />
		</s:elseif>
	</span>
</h1>
	...
	<%-- username --%>
		...
		<div class="form-group<s:property value="#controlGroupErrorClassVar" />">
			<div class="col-xs-12">
				<label for="params_username"><s:text name="label.username" /></label>
				<wpsf:textfield name="username" id="params_username" cssClass="form-control" />
	...
	<%-- password --%>
		...
		<div class="form-group<s:property value="#controlGroupErrorClassVar" />">
			<div class="col-xs-12">
				<label for="params_password"><s:text name="label.password" /></label>
				<wpsf:textfield name="password" id="params_password" cssClass="form-control" />
				<s:if test="#fieldHasFieldErrorVar">
					<span class="help-block text-danger">
						<s:iterator value="%{#fieldFieldErrorsVar}"><s:property />&#32;</s:iterator>
					</span>
	...
	<%-- jenkins_url --%>
		<s:set var="fieldFieldErrorsVar" value="%{fieldErrors['jenkins_url']}" />
		<s:set var="fieldHasFieldErrorVar" value="#fieldFieldErrorsVar != null && !#fieldFieldErrorsVar.isEmpty()" />
		<s:set var="controlGroupErrorClassVar" value="%{#fieldHasFieldErrorVar ? ' has-error' : ''}" />
		<div class="form-group<s:property value="#controlGroupErrorClassVar" />">
			<div class="col-xs-12">
				<label for="params_jenkins_url"><s:text name="label.jenkins_url" /></label>
				<wpsf:textfield name="jenkins_url" id="params_jenkins_url" cssClass="form-control" />
				<s:if test="#fieldHasFieldErrorVar">
					<span class="help-block text-danger">
						<s:iterator value="%{#fieldFieldErrorsVar}"><s:property />&#32;</s:iterator>
					</span>
